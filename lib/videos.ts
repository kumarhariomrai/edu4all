...
function slugify(value: string) {
  return value.toLowerCase().trim().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || 'video';
}
...
function mapRowToVideo(row: string[], headers: string[]): Video {
  ...
  const youtubeId = extractYoutubeId(record.youtube_url || '');
  ...
  return {
    ...
    thumbnail_url: record.thumbnail_url || `https://img.youtube.com/vi/${youtubeId}/hqdefault.jpg`,
    ...
    published: isTrue(record.published),
    featured: isTrue(record.featured),
    display_order: record.display_order || '0',
    published_date: record.published_date || '',
  };
}

async function loadVideosFromSheet(): Promise<Video[]> {
  const spreadsheetId = process.env.GOOGLE_SHEET_ID;
  const sheetName = process.env.GOOGLE_SHEET_NAME || 'Videos';
  const clientEmail = process.env.GOOGLE_SERVICE_ACCOUNT_EMAIL;
  const privateKey = process.env.GOOGLE_PRIVATE_KEY?.replace(/\\n/g, '\n');

  if (!spreadsheetId || !clientEmail || !privateKey) return FALLBACK_VIDEOS;
  ...
  try {
    ...
    return rows
      .slice(1)
      .map((row) => mapRowToVideo(row, headers))
...
export async function getAllVideos() {
  return (await loadVideosFromSheet()).filter((video) => video.published);
}

export async function getVideoBySlug(slug: string) {
  return (await getAllVideos()).find((video) => video.slug === slug) ?? null;
}
